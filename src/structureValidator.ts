...
export function validateCodeStructure(document: vscode.TextDocument, diagnostics: vscode.Diagnostic[]): void {
    ...
    for (let lineIndex = 0; lineIndex < document.lineCount; lineIndex++) {
        ...
        if (typoMatch) {
            ...
            switch (typo) {
                case 'ELSIF': 
                case 'EIF': expectedKeyword = 'ELSEIF'; break;
                case 'ELE': expectedKeyword = 'ELSE'; break;
            }
            const range = new vscode.Range(lineIndex, col, lineIndex, col + typo.length);
            const diagnostic = new vscode.Diagnostic(
                range,
                `Syntaxe incorrecte: "${typo}" n'est pas un mot-clé valide. Utiliser "${expectedKeyword}".`,
        ...
        if (/^(ELSEIF|ELSE|ELIF)\b/i.test(text)) {
            if (blockStack.length === 0 || blockStack[blockStack.length - 1].keyword !== 'IF') {
                ...
                const diagnostic = new vscode.Diagnostic(
                    range,
                    `${text.split(/\s/)[0]} sans IF correspondant.`,
                    vscode.DiagnosticSeverity.Error
                );
                diagnostic.source = 'Sphere Script Linter';
        ...
        if (closingRegex.test(text)) {
            ...
            if (closingKeyword === 'ENIF') {
                ...
                const diagnostic = new vscode.Diagnostic(
                    range,
                    `Syntaxe incorrecte: "${closingKeyword}" n'est pas un mot-clé valide. Utiliser ENDIF.`,
                    vscode.DiagnosticSeverity.Error
                );
                diagnostic.source = 'Sphere Script Linter';
                diagnostic.code = 'invalid-syntax';
            ...
            if (blockStack.length === 0) {
                ...
                const diagnostic = new vscode.Diagnostic(
                    range,
                    `${closingKeyword} sans bloc correspondant.`,
                    vscode.DiagnosticSeverity.Error
                );
                diagnostic.source = 'Sphere Script Linter';
                diagnostic.code = 'mismatched-block';
                ...
                if (lastBlock) { // Vérifier si lastBlock est défini
                    const expectedClosing = closingKeywords[lastBlock.keyword as keyof typeof closingKeywords];
                    ...
                    if (closingKeyword !== expectedClosing) {
                        ...
                        const diagnostic = new vscode.Diagnostic(
                            range,
                            `${closingKeyword} ne correspond pas à ${lastBlock.keyword} à la ligne ${lastBlock.line + 1}.`,
                            vscode.DiagnosticSeverity.Error
                        );
                        diagnostic.source = 'Sphere Script Linter';
                        diagnostic.code = 'mismatched-block';
                    ...
                    const diagnostic = new vscode.Diagnostic(
                        range,
                        `${closingKeyword} sans bloc correspondant (pile vide inattendue).`,
                        vscode.DiagnosticSeverity.Error
                    );
                    diagnostic.source = 'Sphere Script Linter';
                    diagnostic.code = 'mismatched-block';
    ...
    for (const block of blockStack) {
        ...
        const expectedClosing = closingKeywords[block.keyword as keyof typeof closingKeywords];
        const diagnostic = new vscode.Diagnostic(
            range,
            `${block.keyword} à la ligne ${block.line + 1} n'a pas de ${expectedClosing} correspondant.`,
            vscode.DiagnosticSeverity.Error
        );
        diagnostic.source = 'Sphere Script Linter';
