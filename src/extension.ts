    ...
    for (let lineIndex = 0; lineIndex < lineCount; lineIndex++) {
        ...
        if (leadingWhitespaceMatch) {
            ...
            const diagnostic = new vscode.Diagnostic(
                range,
                'Les déclarations de section (ex: [ITEMDEF]) ne doivent pas être précédées par des espaces ou des tabulations.',
        ...
        if (insideWhitespaceMatch) {
            ...
            const diagnostic = new vscode.Diagnostic(
                range,
                'Aucun espace n\'est autorisé entre le crochet  et le mot-clé de la section.',
                vscode.DiagnosticSeverity.Warning
            );
            diagnostic.source = 'Sphere Script Linter';
        ...
        if (currentSection) {
            ...
            if (defnameMatch) {
                ...
                if (defnameValue.toLowerCase() !== currentSection.name.toLowerCase()) {
                    ...
                    const diagnostic = new vscode.Diagnostic(
                        range,
                        `Le DEFNAME \"${defnameValue}\" ne correspond pas à la définition de la section \"${currentSection.name}\".`,
                        vscode.DiagnosticSeverity.Error
                    );
                    diagnostic.source = 'Sphere Script Linter';
...
function* tokenize(text: string): Generator<{ type: string, value: string, index: number }> {
